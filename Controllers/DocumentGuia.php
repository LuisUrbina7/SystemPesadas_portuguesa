...
class DocumentGuia extends Controllers
{
	...
	public function details()
	{
		...
		$data['page_data'] = $this->model->getDetails($_GET['id'], $_GET['tr'], $_SESSION['userData']['OPE_AMC_PESADA']);

		$data['page_dcg_numero'] = !isset($_GET['id']) ? 'S/R' : $_GET['id'];
		$data['page_tra_codigo'] = !isset($_GET['tr']) ? 'S/R' : $_GET['tr'];
		$data['page_ind'] = !isset($_GET['ind']) ? '0' : $_GET['ind'];
		$data['page_canastas'] = $this->model->getCanasta();
		$data['page_amc'] = $this->model->getAmc();


		$data['page_functions_js'] = "functions_details_guia.js";
