...
async function Import() {
	...
	document.getElementById('pdc_clt').value = idData;
...
function fill(data) {
	...
	let idData = document.getElementById('pdc_clt').value;


	tableBodyDoc.innerHTML = data.map(data => `
			 <li class="list-group-item d-flex justify-content-between align-items-center border" onclick="docSelect(this)" data-client="${idData}" data-doc="${data.DCL_NUMERO}" data-tipodoc="${data.TIPODOC}">
                                    ${data.DCL_NUMERO}
...
async function docSelect(doc) {
	...
	let idData = doc.getAttribute('data-client');
