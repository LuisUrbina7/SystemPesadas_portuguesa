...
function getWeight() {
	...
	var ajaxUrl = base_url + '/DocumentGuia/getWeight';
	...
	if (id) {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				var ajaxUrl = base_url + '/DocumentGuia/delete?id=' + id;
...
function setInfo(data) {
	...
	if (data != null) {
		...
		ajaxUrl = base_url + "/DocumentGuia/content?id=" + dpvNumber + "&tr=" + dpvProvider + "&pdt=" + pdtCodigo;
...
function insertDetails() {
	...
	if (pdtCodigo != '0000') {
		...
		var ajaxUrl = base_url + '/DocumentGuia/insertDetails';
...
function updateDetails(event) {
	...
	if (event.keyCode == 13) {
		...
		var ajaxUrl = base_url + '/DocumentGuia/updateDetails';
...
function closeCount() {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			var ajaxUrl = base_url + '/DocumentGuia/getDetailsToEdit';
			...
			request.onreadystatechange = function () {
				...
				if (request.status == 200) {
					...
					if (objData.status) {
						...
						add();
						window.location.href = base_url + '/DocumentGuia/document';
					} else {
...
function ignoreHeavy(data) {
	...
	swalWithBootstrapButtons.fire({
		...
		if (result.isConfirmed) {
			...
			var ajaxUrl = base_url + '/DocumentGuia/insertSkip';
...
function seeLots() {
	...
	var ajaxUrl = base_url + '/DocumentGuia/seeLots?pdt=' + pdtCode + '&und=' + pdtUnd;
...
function reportDetailsPesada() {
	...
	var ajaxUrl =
		base_url +
		"/DocumentGuia/viewPdfDetailsPesada?id=" +
		dclCodigo +
		"&tr=" +
		traCodigo +
		"&pdt=" +
...
async function cut() {
	...
	try {
		...
		if (result.isConfirmed) {
			...
			const ajaxUrl = base_url + '/DocumentGuia/cut';
...
function ticketByHeavy(data) {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			var ajaxUrl = base_url + '/DocumentGuia/ticketByHeavy?id=' + dclCodigo + '&tr=' + traCodigo + '&pdt=' + pdtCodigo;
