...
class DocumentGuiaModel extends Mysql
{
	...
	public function getDocument()
	{
		...
		// $document = "SELECT DCG_NUMERO,DCG_VEH_PLACA, DCG_TRA_CODIGO, TRA_NOMBRE, DCG_NETOGUIA, DCG_FECHA, DCG_ESTADO, DCG_PESADAS, CONCAT(DCG_NUMERO,':',DCG_VEH_PLACA,':',DCG_TRA_CODIGO,':',DCG_FECHA) AS INDICE FROM ADN_DOCCLIGUIA JOIN ADN_DOCCLI ON DCG_NUMERO = DCL_NUMGUIA JOIN ADN_TRANSPORTISTAS ON DCG_TRA_CODIGO = TRA_CODIGO WHERE  DCG_ESTADO = 0  AND DCL_TDT_CODIGO IN('PEDW','FAV') GROUP BY INDICE ORDER BY DCG_FECHA DESC LIMIT 15 ;";
		/*	$document = "SELECT DCG_NUMERO,
		DCG_VEH_PLACA, 
		DCG_TRA_CODIGO, 
		TRA_NOMBRE, 
		ROUND(SUM(UGR_PESO*MCL_CANTIDAD*MCL_CANTXUND*MCL_ACTIVO) / 1000,2) AS DCG_PESO, 
		DCG_FECHA, 
		DCG_ESTADO, 
		DCG_PESADAS,
		...
		IF(DCL_TDT_CODIGO NOT IN ('PED','PEDW'), 1,0) AS INDICADOR
	FROM ADN_MOVCLI 
		...
		JOIN `ADN_DOCCLIGUIA` ON DCG_NUMERO = DCL_NUMGUIA
		JOIN ADN_TRANSPORTISTAS ON DCG_TRA_CODIGO = TRA_CODIGO LEFT JOIN (
			   ...
			   ) AS PESADAS ON DCG_NUMERO = PDA_NUMERO AND DCG_TRA_CODIGO = PDA_DET_CODIGO AND DCL_SCS_CODIGO = PDA_SCS_CODIGO
		JOIN ADN_UNDAGRU ON MCL_UPP_UND_ID = UGR_UND_ID AND MCL_UPP_PDT_CODIGO = UGR_PDT_CODIGO
		 WHERE  DCG_ESTADO = 0   
