...
class PedidosPKModel extends Mysql
{
...
AND DCL_TIPTRA = 'D' 
AND DCL_STD_ESTADO = 'PEN'
AND DCL_NUMERO<>''
AND DCL_ACTIVO = '1'
AND DCL_TDT_CODIGO IN('PEDW','PED','PEDW2','PEDW3')
INNER JOIN (
	SELECT 
	...
	AND MCL_EXPORT = '0'
	 ...
	 AND (MCL_CANTIDAD= '0' OR DCL_COMEN2 LIKE '%:P:%' )
...
ORDER BY DCL_FECHAHORA DESC;";
	...
	public function getDetails($number, $vnd, $amc, $tdoc)
	{
		...
		$details = "SELECT STRAIGHT_JOIN
						...
						TIPO
							...
							INNER JOIN (
								...
								FROM ADN_DOCCLI 
								WHERE DCL_TIPTRA = 'D'
								AND DCL_NUMERO = '$number'
								AND DCL_TDT_CODIGO = '$tdoc') AS DOC
							...
							WHERE DCL_TIPTRA = 'D'
							AND DCL_TDT_CODIGO = '$tdoc'
							AND MCL_EXPORT = 0
							AND (MCL_CANTIDAD = 0 OR MCL_AMPLIO LIKE '%CTD-OLD%')
	...
	public function validateDocumento($numero, $vnd,$tdoc)
	{
		...
		JOIN (
			...
			FROM (
				...
				WHERE DCL_NUMERO = '$numero' AND DCL_VEN_CODIGO = '$vnd' AND DCL_TDT_CODIGO = '$tdoc'
				AND MCL_CANTIDAD =0 AND MCL_EXPORT = 0
				GROUP BY MCL_UPP_PDT_CODIGO
			) AS DOCUMENTOS
	...
	public function callUpdateAmount($numero,$codigo,$tipo){
		...
		$procedure = "CALL POSTCERRAR_PESADA_PK('$numero','$codigo','$tipo')";
